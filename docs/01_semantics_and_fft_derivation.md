...
**上限约束**：~~首版本计划限制 `N_fft <= 8192`~~ →
**实际交付值为 `N_fft <= 16384`**（`FFT_CONV1D_MAX_NFFT`，见 `op_host/fft_conv1d_tiling.h`）。

这里的差异来自阶段 3 的数据流改动，记录在案：

- 阶段 2 设计稿把频谱/旋转因子/kernel 频谱全部常驻 UB，`N=8192` 时旋转因子 64KB +
  kernel 频谱 64KB 已占 128KB/192KB，开不起 ping-pong，故卡在 8192。
- v1 实现改为**中间结果全部落 workspace GM**，Vector 按 2048 float 分块处理，
  UB 占用固定约 60KB 且与 `N` 无关，上述约束不再成立。
- v1 的实际限制变成"`N` 为 4 的幂且常量表能 L1 常驻"：
  `N=16384 → N1=128 →` D 表 128KB + T 表 128KB = 256KB ≤ 448KB ✔；
  `N=65536 → N1=256 →` 仅 D 表就 512KB ✗。故取 16384。
- 需要说明：在 GM 数据流下 L1 常驻属性能考虑而非正确性要求，
  16384 是 v1 的**范围线**而非硬件硬墙；唯一硬约束是 workspace
  （`N=16384, H=16, 20 核` 约 15.5MB）。放宽只需改这个常量，代价是
  `N1=256` 的 GEMM 与二次增长的常量表开销。

超出上限时 host 校验会打印原因并返回失败；后续可走分块卷积（overlap-save）或 direct 路径。
...
| N_fft 上限 | **16384**（`FFT_CONV1D_MAX_NFFT`，等价于 `L+K-1 <= 16384`） | host 校验（带原因打印） |
