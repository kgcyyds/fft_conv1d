...
当前 AscendC Host 为了得到方形四步分解，取不小于
`max(2, L+K-1)` 的最小 **4 的幂**：

```
N_fft = 4^⌈log4(max(2, L+K-1))⌉
N1 = N2 = sqrt(N_fft)
```

Python 阶段 1 原型仍保留“最小 2 的幂”的通用参考实现；它用于验证卷积语义，
不代表当前设备侧分派。
...
| 条件 | 路径 | FFT 长度 |
|---|---|---|
| `K < 64` | DIRECT | 只为 tiling 记录计算 `N_fft`，不执行 FFT |
| `K >= 64` 且 `L+K-1 <= 1024` | FFT-UB | `N_fft <= 1024` |
| `K >= 64` 且 `1024 < L+K-1 <= 4096` | FFT-GM | `N_fft = 4096` |
| `L+K-1 > 4096` | DIRECT 回退 | 不执行 FFT |

因此 4096 是**当前 FFT 路径上限**，不是算子长度硬上限；更大 shape 不会因 FFT
容量返回失败，而是回退到数值正确但更慢的 DIRECT。
...
| 设备侧 N_fft | 不小于 `max(2,L+K-1)` 的最小 4 的幂 | Host `NextPow4` |
| 算法分派 | `K<64` 走 DIRECT；FFT-UB 上限 1024；FFT-GM 上限 4096；更大回退 DIRECT | Host tiling |
| 内存 | 连续（contiguous），地址按硬件要求对齐 | 待阶段 3 |
| 语义 mode | 设备算子只有 `math`；Python 原型保留 `corr` 用于对照 | Python 参考/设备接口 |
